...
//        model.addAttribute("employees", employees);
    ...
    public String upload(@RequestParam("file") MultipartFile file, @PathVariable("lastName") String lastName) {
        try {
            if (file.isEmpty()) {
                return "文件为空";
            }
            ...
            if (!FileUtils.checkFileSize(file, 50, "M")) {
                log.error("上传文件规定小于500MB");
                return "上传文件规定小于500MB";
            }
            // 获取文件名
...
//            fileOwn.setId(1);
    ...
    public String downloadFile(HttpServletRequest request, HttpServletResponse response, @PathVariable("remark") String remark)
            throws UnsupportedEncodingException {
        String fileName = remark;// 文件名room.zip
        String result = FileUtils.downLoad(response, fileName);
        if (request == null) {
            return null;
        }
        return result;
...
//        for (FileOwn file : files) {
//            resStr.append(file.getRemark()+" || ");
//        }
//        String res = resStr.toString().substring(0,resStr.length()-3);
